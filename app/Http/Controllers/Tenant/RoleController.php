...
class RoleController extends Controller
{
    public function index()
    {
        ...
        if($user->hasRole(['Admin', 'Owner'])) {
            $lims_role_all = Role::all();
            return view('Tenant.role.create', compact('lims_role_all'));
        }
        else
    ...
    public function permission($id)
    {
        ...
        if ($user->hasRole(['Admin', 'Owner'])) {
            ...
            if (!$role) {
                return redirect()->back()->with('not_permitted', 'Role not found.');
            }
            $permissions = $role->permissions;
            $all_permission = $permissions->pluck('name')->toArray();

            if (empty($all_permission)) {
                $all_permission[] = 'No permissions assigned';
            }
            return view('Tenant.role.permission', [
                'role' => $role,
    ...
    public function destroy($id)
    {
        ...
        return redirect('role')->with('not_permitted', 'Data changed successfully');
    }
